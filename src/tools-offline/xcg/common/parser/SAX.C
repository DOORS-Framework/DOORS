...
#include <doors/parserInternals.h>
#include <doors/valid.h>
#include <doors/entities.h>
#include <doors/xmlerror.h>
#include <doors/xmlIO.h>
#include <doors/SAX.h>
#include <doors/uri.h>
#include <doors/valid.h>
 ...
 ****************/
    ...
    if (ns != NULL)
	namespace_ = xmlSearchNs(ctxt->myDoc, ctxt->node, ns);
    else {
	namespace_ = NULL;
    }

    /* !!!!!! <a toto:arg="" xmlns:toto="http://toto.com"> */
    ret = xmlNewNsProp(ctxt->node, namespace_, name, NULL);
...
startElement(void *ctx, const xmlChar *fullname, const xmlChar **atts)
{
    ...
    if (ctxt->linenumbers) {
	if (ctxt->input != NULL)
	    ret->content = (xmlChar*) ctxt->input->line;
    }
...
int
checkNamespace(void *ctx, xmlChar *namespace_)
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    xmlNodePtr cur = ctxt->node;
    ...
    if (namespace_ == NULL) {
		...
		ctxt->sax->error(ctxt, 
		 "End tags %s holds a prefix %s not used by the open tag\n",
		                 cur->name, namespace_);
	    ctxt->wellFormed = 0;
	} else if (!xmlStrEqual(namespace_, cur->ns->prefix)) {
	    if ((ctxt->sax != NULL) && (ctxt->sax->error != NULL))
		ctxt->sax->error(ctxt, 
    "Start and End tags for %s don't use the same namespaces: %s and %s\n",
	                         cur->name, cur->ns->prefix, namespace_);
	    ctxt->wellFormed = 0;
	} else
	    return(1);
    }
