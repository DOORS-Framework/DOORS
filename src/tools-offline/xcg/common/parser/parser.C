...
#include <doors/encoding.h>
#include <doors/xmlIO.h>
#include <doors/uri.h>
#ifdef LIBXML_CATALOG_ENABLED
#include <doors/catalog.h>
#endif
...
xmlParseConditionalSections(xmlParserCtxtPtr ctxt) {
  ...
  if ((RAW == 'I') && (NXT(1) == 'N') && (NXT(2) == 'C') &&
	...
	ctxt->instate = (xmlParserInputState)instate;
...
xmlParseContent(xmlParserCtxtPtr ctxt) {
  ...
  while (((RAW != 0) || (ctxt->token != 0)) &&
	...
	if (ctxt->token != 0) {
	  ...
	  xmlEntityPtr e = xmlParseEntityRef(ctxt);
	  if( e && e->name && e->name[0] == 'l'
		  && e->name[1] == 't'  && e->name[2] == 0 ) {
		ctxt->input->cur++;
		ctxt->input->cur++;
		ctxt->input->cur++;
		ctxt->input->cur++;
		ctxt->sax->characters(ctxt,(const xmlChar*)"<",1);
	  }
	}
